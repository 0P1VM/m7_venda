...
const gerarPagamento = async (interaction) => {
    ...
    const canalLogsCompras = interaction.guild.channels.cache.get(canalLogs);
    ...
    try {
        ...
        if (!carrinhoDados || carrinhoDados.produtos.length < 1) {
            return interaction.reply('Não tem nada no carrinho!').then(() => {
                setTimeout(() => {
        ...
        const valor = carrinhoDados.produtos.reduce((acc, p) => acc + p.produto_valor, 0);
        const nomesProdutos = [...new Set(carrinhoDados.produtos.map(p => p.produto_nome))].join(' | ');
        ...
        const aguardandoPagamentoRow = new MessageActionRow()
            .addComponents(
                new MessageButton()
        ...
        const msgsProduto = (await interaction.channel.messages.fetch()).filter(msg => idMsgsProduto.includes(msg.id));
        interaction.channel.bulkDelete(msgsProduto).catch(() => {});

        const email = 'emailqualquer@sla.com';
        const payment_data = {
        ...
        coletorCopiaCola.on('collect', async i => {
            i.channel.send({ content: `${data.body.point_of_interaction.transaction_data.qr_code}` })
                .then(m => msgsApagar.push(m.id));
        ...
        const interval = setInterval(async () => {
            ...
            if (tentativas >= 8 || pagamentoStatus === 'approved') {
                ...
                if (pagamentoStatus === 'approved') {
                    ...
                    await interaction.message.edit({ components: [aguardandoPagamentoRow] });
                    ...
                    if (tamanhoConteudo < 2000) {
                        await interaction.channel.send(`\`${conteudoProdutos.join('\n')}\``);
                    ...
                    // Entrega de produtos que ultrapassem 2048 caracteres do Discord NÃO TESTADA
                    const [conteudoSeparadoP1, conteudoSeparadoP2] = [
                        conteudoProdutos.slice(0, Math.ceil(conteudoProdutos.length / 2)),
                        conteudoProdutos.slice(Math.ceil(conteudoProdutos.length / 2))
                    ];

                    await interaction.channel.send(conteudoSeparadoP1.join('\n'));
                    await interaction.channel.send(conteudoSeparadoP2.join('\n')).then(async () => {
                    ...
                    interaction.channel.send({
                        content: `${interaction.user}, caso seu produto não foi entregue automaticamente, clique no botão abaixo para verificar o pagamento`,
                        components: [
