...
const iniciarCompra = async (interaction, categoriaCarrinho) => {
    ...
    if (!itemEncontrado) {
        return interaction.reply({ content: 'Esse produto não está cadastrado no sistema', ephemeral: true });
    }
    ...
    if (!produtoEscolhido) {
        return interaction.reply({ content: `${interaction.user} não há produtos \`${nome}\` no estoque`, ephemeral: true });
    }
    ...
    const produtoNoCarrinho = await Carrinho.findOne({ ...filtroCarrinho, 'produtos.produto_id': _id });

    if (produtoNoCarrinho) {
        return interaction.followUp({ content: `${interaction.user}, esse produto já está no carrinho, selecione a quantidade desejada em ${carrinhoCanal}`, ephemeral: true });
    }

    const msgProduto = await carrinhoCanal.send({
        embeds: [atualizarEmbedQtdProduto(nome)],
        components: [
            new MessageActionRow().addComponents(
                new MessageButton().setLabel('Adicionar').setStyle('PRIMARY').setCustomId(`adicionar_produto_${_id}`),
                new MessageButton().setLabel('Remover').setStyle('DANGER').setCustomId(`remover_produto_${_id}`)
            )
        ]
    });
    ...
    await ProdutoEstoque.deleteOne({ produtoId: _id, server_id: interaction.guildId, conteudo: produtoEscolhido.conteudo });

    const quantidade = await ProdutoEstoque.countDocuments({ produtoId: _id, server_id: interaction.guildId });

    const produtoAtualizado = await Produto.findOneAndUpdate({ _id, server_id: interaction.guildId }, { quantidade }, { returnDocument: 'after' });
    ...
    try {
        ...
        return carrinhoCanal.send('Erro ao processar dados do carrinho');
    }

    await msgCarrinhoStatus.edit({ embeds: [gerarEmbedCarrinhoDetalhes(carrinhoDados.produtos.map(p => ({ nome: p.produto_nome, valor: p.produto_valor })), interaction)] });
};
