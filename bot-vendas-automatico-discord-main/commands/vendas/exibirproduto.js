...
const run = async (client, message) => {
    ...
    const menuCollector = message.channel.createMessageComponentCollector({
        filter: i => i.customId === 'menu_produtos' && i.user.id === message.author.id,
    ...
    menuCollector.on('collect', async i => {
        ...
        await Produto.updateOne({ _id: itemSelecionado._id }, { quantidade: itemSelecionado.quantidade });
    ...
    menuCollector.on('end', collected => {
        if (collected.size === 0) {
            ...
            message.channel.send('Tempo esgotado, por favor tente novamente.');
        }
    });
};
